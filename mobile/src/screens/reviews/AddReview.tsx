...
import * as reviewSlice from "@/features/reviews/reviewsSlice";
...
export default function AddReviewScreen() {
  ...
  const onSubmit = async ({ rating, comment }: { rating: number; comment: string }) => {
    await dispatch((reviewSlice as any).submitOrderReview({ orderId: Number(orderId), vendorId: Number(vendorId), rating, comment } as any));
    router.back();
  };
  ...
  return (
    <Screen>
      ...
      <ReviewForm onSubmit={(onSubmit as any)} />
    </Screen>
  );
}
