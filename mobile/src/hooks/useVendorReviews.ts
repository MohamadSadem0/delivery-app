...
import { selectVendorSummary, selectVendorReviews, selectVendorReviewsStatus } from '@/features/reviews/reviews.selectors';
...
export function useVendorReviews(vendorId: number) {
  ...
  useEffect(() => { if (vendorId) dispatch(fetchVendorReviews((vendorId) as any)); }, [vendorId, dispatch]);
