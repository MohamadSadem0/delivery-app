...
export default function NotificationListItem({ item, onPress }: Props) {
  ...
  return (
    <Pressable onPress={onPress}>
      <View style={{ paddingVertical: spacing.md, paddingHorizontal: spacing.sm, backgroundColor: item.isRead ? 'transparent' : colors.surface, borderRadius: radii.md }}>
        <Text weight={item.isRead ? 'regular' : 'semiBold'}>{item.title}</Text>
