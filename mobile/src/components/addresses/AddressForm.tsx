...
type Form = {
  ...
  line1: string; line2?: string | null; area?: string; city?: string; governorate?: string; phone?: string; instructions?: string;
};
...
export default function AddressForm({ initial, onSubmit, submitLabel = 'Save' }: { initial?: Partial<Form>; onSubmit: (v: Form) => void; submitLabel?: string }) {
  ...
  return (
    <View style={{ gap: spacing.sm }}>
      ...
      <TextInput value={v.label} onChangeText={(t)=>setV({...v,label:t})} placeholder="Home, Workâ€¦" placeholderTextColor={colors.textMuted} style={input}/>
      ...
      <TextInput value={v.line2||''} onChangeText={(t)=>setV({...v,line2:t})} placeholder="Apartment, floorâ€¦" placeholderTextColor={colors.textMuted} style={input}/>
      ...
      <TextInput value={v.instructions||''} onChangeText={(t)=>setV({...v,instructions:t})} placeholder="Gate code, landmarksâ€¦" placeholderTextColor={colors.textMuted} style={[input,{minHeight:88}]} multiline/>
